...
class ShipForm2(FormAction):
    ...
    def submit(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict]:
        ...
        dispatcher.utter_message(text="%s %s의 정보\n항명: %s\n호출부호: %s\n선명: %s\n외내: %s\n입출: %s\n총톤수: %s\n"
                                      "입항일시: %s\n출항일시: %s\n국적: %s\n선박용도: %s" % (start, shipNameFT,request.findInfo(a, 'prtAgNm'),
                                    request.findInfo(a, 'clsgn'),request.findInfo(a, 'vsslNm'),request.findInfo(a, 'ibobprtSeNm')
                                    ,request.findInfo(a, 'etryndNm'),request.findInfo(a, 'grtg'),request.findInfo(a, 'etryptDt'),request.findInfo(a, 'tkoffDt')
                                    ,request.findInfo(a, 'vsslNltyNm'),request.findInfo(a, 'vsslKindNm')))
